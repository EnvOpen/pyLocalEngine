...
def benchmark_translation_lookup():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(f"  Simple key lookup: {statistics.mean(simple_times)*1000:.2f}ms ± {statistics.stdev(simple_times)*1000:.2f}ms")
            print(f"  Nested key lookup: {statistics.mean(nested_times)*1000:.2f}ms ± {statistics.stdev(nested_times)*1000:.2f}ms")
            print(f"  Missing key (default): {statistics.mean(missing_times)*1000:.2f}ms ± {statistics.stdev(missing_times)*1000:.2f}ms")
...
def benchmark_locale_switching():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(f"  Cached locale switch: {statistics.mean(cached_times)*1000:.2f}ms ± {statistics.stdev(cached_times)*1000:.2f}ms")
            print(f"  Fresh locale switch: {statistics.mean(fresh_times)*1000:.2f}ms ± {statistics.stdev(fresh_times)*1000:.2f}ms")
        ...
        try:
            ...
            for num_threads in [1, 2, 4, 8]:
                with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
                    ...
                    print(f"  {num_threads} threads: {total_time:.2f}s total, {avg_time:.2f}ms avg per operation")
